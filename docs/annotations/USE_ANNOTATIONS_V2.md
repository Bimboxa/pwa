...
2. **Never remove the scoped `count()` observation reads** at the top of the liveQuery callback. They look useless; they are what keeps every instance's liveQuery *observing* db.annotations/db.points now that the heavy reads are served from shared module caches (Dexie only re-runs a liveQuery on writes to tables/ranges its callback actually read). They must stay **scoped** (by baseMapId / listingId / projectId): unfiltered `count()` scans the whole table across every project — measured ~140ms per call on a large multi-project local DB, serializing the commit waves.
...
- **Observation keep-alive**: every callback fires tracked reads on db.annotations and db.points, **scoped to the instance's own range** (baseMapIds / listingId / projectId) and **minimal** (`limit(1).primaryKeys()` — Dexie registers the queried RANGE regardless of limit, while `count()` walks the whole index range: 14 counts serialized ~500ms per commit wave on slow-IDB machines). They are fired at the top of the callback but only awaited at its end, so they overlap the shared-cache work. Scoped is not optional: a full-table read observes every project's rows and gets expensive on old local DBs. Non-indexed-field updates are covered by the separate `_dbWriteTick` React-side dep (bumped by the storagemutated listener).
- **Per-annotation resolve memo** (`_resolvedRowsCache`, WeakMap): the geometric resolution (resolvePoints/cuts, guideLine ramp, stage-A qties) is memoized per annotation row, keyed on the identity of the row + its point rows + baseMap scalars, with one variant per `withQties` flag. Cached objects are returned as shallow copies so later in-place field assignments (proxies, profile helpers) never poison the cache. Log: `resolve memo X/Y` in the points/qties line.
- **Same-tab invalidation**: Dexie table hooks (`creating/updating/deleting`) fire synchronously during the write, *before* any liveQuery re-run reads the cache. Points: per-id delete + generation bump — **including `creating`**: a point id can be negatively cached (`null` = "no such row") when a read raced its creation, and the creating hook is what drops that stale null.
- **Annotations rows: incremental PATCHING, not clearing** (`_recordAnnotationWrite`): the hooks accumulate the touched primary keys per ROOT transaction; on `transaction.on("complete")`, ONE `db.annotations.bulkGet(keys)` re-reads the committed rows (audit stamps included, soft-deletes visible as `deletedAt`) and patches every cached array according to its query scope (`baseMaps:`/`listing:`/`project:`). Batch writes (bulkAdd/bulkPut/bulkDelete/mass updates — procedure saves, mass deletes…) therefore cost a single bulkGet + one array rebuild per cache entry, whatever the row count. A synchronous **barrier** (`_annPatchBarriers`) makes any liveQuery re-run triggered by the same commit wait for the patch (otherwise it would read the pre-write rows and stay stale). Fallbacks to a full clear: patch failure, > 500 keys in one transaction, hook without transaction, foreign (cross-tab) mutations. This removed the last big stage-A cost (the full range query re-ran on every commit: ~200ms on heavy maps / slow IDB).
