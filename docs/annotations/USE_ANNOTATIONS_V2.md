...
- **Observation keep-alive**: every callback starts with tracked `count()` reads on db.annotations and db.points, **scoped to the instance's own range** (baseMapIds / listingId / projectId — same observation semantics as the pre-#290 direct reads). Scoped is not optional: a full-table `count()` scans every project's rows and gets expensive on old local DBs.
