...
function computeDevelopedPerimeter(
  ring,
  meterByPx,
  isClosed,
  hiddenSegmentsIdx = []
...
function computeWallQty(points, meterByPx, hiddenSegmentsIdx = []) {
  ...
  for (let i = 0; i < n - 1; i++) {
    ...
    length += Math.hypot(dx, dy, bottomB - bottomA);
    surface += Math.hypot(dx, dy) * ((hA + hB) / 2);
  }
...
function getArcMetrics(p0, p1, p2) {
  try {
    ...
    if (sagitta < chordLength * 0.001 || chordLength < 1e-9) {
      return {
        length: getDistance(p0, p1) + getDistance(p1, p2),
        chordArea: 0,
        isFlat: true,
      };
    }
    ...
    const totalTheta =
      Math.abs(normalize(a1 - a0)) + Math.abs(normalize(a2 - a1));
    ...
    return {
      length: arcLength,
      chordArea: cross > 0 ? chordArea : -chordArea,
      isFlat: false,
    };
  } catch (e) {
    return {
...
export default function getAnnotationQties({
  ...
  try {
    ...
    if (annotation.type === "STRIP") {
      ...
      const surface = polygons.reduce((acc, polygon) => {
        const qty = getAnnotationQties({
          annotation: { ...polygon, type: "POLYGON" },
          meterByPx,
        });
    ...
    const closeLine =
      annotation.type === "POLYGON" || annotation.closeLine || false;
        ...
        while (i < limit && iterations < MAX_ITER) {
          ...
          if (isArc) {
            ...
            if (metrics.isFlat) {
              ...
              if (
                !hiddenSegments.includes(i0) &&
                !hiddenSegments.includes(i1)
              ) {
    ...
    const mainMetrics = calculatePathMetrics(
      points,
      closeLine,
    ...
    if (closeLine && Array.isArray(annotation.cuts)) {
      annotation.cuts.forEach((cut) => {
        const cutPoints = (cut.points || []).filter(
          (p) => p && typeof p.x === "number"
        );
        if (cutPoints.length >= 3) {
          const cutMetrics = calculatePathMetrics(
            cutPoints,
            true,
    ...
    if (annotation.type === "POLYLINE") {
      ...
      if (hasPerVertexZOffsets(annotation)) {
        ...
        return {
          ...
          lengthDeveloped: wall.length,
          surfaceDeveloped: wall.surface,
        };
      }

      if (annotation.height) {
        totalSurfacePx =
          mainMetrics.lengthPx * (parseFloat(annotation.height) / meterByPx);
      }
    }
    ...
    if (closeLine && hasPerVertexZOffsets(annotation)) {
      ...
      const tri = triangulateAnnotationGeometry({
        ...
        isoBandLevels: annotation?.guideLines?.some(
          (g) => g?.points?.length >= 2
        )
          ? ISO_BAND_LEVELS
          : 0,
