...
const SmartDetectLayer = forwardRef(({
    ...
    const analyzeImageThrottled = useMemo(() => throttle(async (imageUrl, currentRoi) => {
        ...
        try {
            ...
            // --- MAIN RECTANGLE ---
            if (result.mainRectangle.found) {
                setMainRectangle(result.mainRectangle);
            } else {
    ...
    return (
        <Box sx={{ width: 1, display: "flex" }} ref={containerRef}>
            <div
                //ref={containerRef}
                style={{
                ...
                <svg width="100%" height="100%" style={{ position: 'absolute', top: 0, left: 0 }}>
                    ...
                    {mainRectangle && (
                        <polygon
                            ...
                            fill="rgba(255, 215, 0, 0.15)" // Or léger transparent
                            stroke="#ee00ffff" // 
                            strokeWidth="1"
                            vectorEffect="non-scaling-stroke"
                            strokeDasharray="4" // Pointillés pour différencier des lignes solides
                        />
                    )}
            ...
            <Box>
                <List>
                    {detectModes.map(mode => {
                        const selected = mode.key === selectedDetectMode;
                        return <ListItem>
                            <Typography sx={{ fontWeight: selected ? "bold" : "normal" }}>{mode.label}</Typography>
                        </ListItem>
                    })}
                </List>
            </Box>
