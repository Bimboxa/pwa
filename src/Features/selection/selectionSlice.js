  ...
  reducers: {
    ...
    triggerSelectionBack: (state) => {
      ...
      if (item) {
        ...
        else if (item.type === "PORTFOLIO_PAGE") {
          item.type = "PORTFOLIO";
          item.id = item.portfolioId;
        }
        else if (
          item.type === "PORTFOLIO_TITLE" ||
          item.type === "PORTFOLIO_DETAIL_REF"
        ) {
          // page title / detail reference elements: back to their page
          // (item.id is the page id)
