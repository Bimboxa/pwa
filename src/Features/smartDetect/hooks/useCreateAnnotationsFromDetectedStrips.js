...
/**
 * Bulk-create STRIP annotations from the result of detectStripFromLoupe
 * (or detectSimilarStrips). Same pattern as useSplitAnnotationsInSegments:
 ...
 * fillColor, etc.) but its `stripOrientation` is taken from the detection
 * result (computed per strip so the body covers the dark wall pixels).
 *
 * Input:
 *   {
 *     strips: Array<{
  ...
  return async ({ strips, sourceAnnotation }) => {
    ...
    for (const strip of strips) {
      ...
      allAnnotations.push({
        ...
        // stripOrientation comes from the detection (per strip); fall back
        // to the template value if the algorithm didn't supply one.
        stripOrientation: strip.stripOrientation ?? templateStripOrientation,
