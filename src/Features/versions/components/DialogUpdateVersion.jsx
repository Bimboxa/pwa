...
import { updateVersion } from "../versionsSlice";
...
export default function DialogUpdateVersion({ open, onClose }) {
  const dispatch = useDispatch();

  // strings

  const title = "Mise à jour de la version";
  ...
  console.log("debug_2110_updatedVersion", version);
  ...
  useEffect(() => {
    if (version?.id) {
      setLabel(version?.mediaMetadata?.label);
      setAuthor(version?.mediaMetadata?.author);
      setDescription(version?.mediaMetadata?.description);
    }
  }, [version?.id]);
  ...
  async function handleCreate() {
    if (loading) return;
    setLoading(true);
    const krtoVersion = await updateKrtoVersionService({
      orgaCode,
      projectId,
      label,
      author,
    ...
    console.log("updated krtoVersion", krtoVersion);
    dispatch(updateVersion(krtoVersion?.metadata));
