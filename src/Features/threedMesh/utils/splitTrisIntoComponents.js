 ...
 * @param {(p, q) => boolean} [args.isSeamEdge] - edges answering true are cut
 *   open: they never connect two triangles (see seamUtils)
 * @returns {number[][]} edge-connected components (original tri indices)
 */
export default function splitTrisIntoComponents({
  positions,
  index,
  tris,
  isSeamEdge,
  ...
  const seamEdges = isSeamEdge ? new Set() : null;
  for (let local = 0; local < tris.length; local++) {
    ...
    for (let e = 0; e < 3; e++) {
      ...
      if (seamEdges && !seamEdges.has(key)) {
        const p = getPos(vertIndex(t, e));
        const q = getPos(vertIndex(t, (e + 1) % 3));
        if (isSeamEdge(p, q)) {
          seamEdges.add(key);
          continue;
        }
      } else if (seamEdges?.has(key)) {
        continue;
      }
