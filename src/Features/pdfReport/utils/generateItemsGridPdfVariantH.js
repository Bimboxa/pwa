...
import drawTitleBlockOnPdfPage from "Features/titleBlocks/utils/drawTitleBlockOnPdfPage";
import sanitizeText from "Features/titleBlocks/utils/sanitizeWinAnsiText";
 ...
 * opts.titleBlock = { manifest, values, bindings, labelOverrides }
 *                                      // full title block header (cartouche)
 *
...
export default async function generateItemsGridPdfVariantH(items, opts = {}) {
  ...
  // ==== Title block (cartouche) header, shared with the portfolio editor
  const titleBlock = opts.titleBlock; // { manifest, values, bindings, labelOverrides }
  ...
  function addPageWithHeader() {
    ...
    if (titleBlock?.manifest) {
      ...
      const layoutData = computeTitleBlockLayout(manifest, rect, {
        ...
        bindings: titleBlock.bindings,
        labelOverrides: titleBlock.labelOverrides,
      });
      // page number cell left empty — filled globally after merge
      drawTitleBlockOnPdfPage(page, {
        layoutData,
